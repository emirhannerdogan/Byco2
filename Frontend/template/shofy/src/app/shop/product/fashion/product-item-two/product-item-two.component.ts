...
export class ProductItemTwoComponent {
  ...
  formatPrice(price: number): string {
    return price
      .toFixed(2) // İki ondalık haneye yuvarla
      .replace('.', ',') // Ondalık ayracı olarak virgül kullan
      .replace(/\B(?=(\d{3})+(?!\d))/g, '.'); // Binlik ayrımlar için nokta ekle
  }
  getFirstImage(imgString: string): string {
    return imgString.split(/(?=https:\/\/)/g)[0]; // İlk `https://` URL'sini al
  }
}
